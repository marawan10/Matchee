...
// Import the new football-data.org API
import { getFootballDataMatches } from './football-data-api'

const API_BASE_URL = 'https://api-football-v1.p.rapidapi.com/v3'

// API configuration - users will need to add their own API key
const apiConfig = {
  headers: {
    'X-RapidAPI-Key': process.env.NEXT_PUBLIC_RAPIDAPI_KEY || 'YOUR_API_KEY_HERE',
...
export async function getMatches(dateFrom?: string, dateTo?: string): Promise<Match[]> {
  // Try football-data.org API first (free and easier)
  try {
    return await getFootballDataMatches(dateFrom, dateTo)
  } catch (error) {
    console.error('Failed to fetch from football-data.org, trying RapidAPI...', error)
    
    // Fallback to RapidAPI if available
    try {
      if (process.env.NEXT_PUBLIC_RAPIDAPI_KEY && process.env.NEXT_PUBLIC_RAPIDAPI_KEY !== 'YOUR_API_KEY_HERE') {
    ...
    console.log('Using mock data - add API key to .env.local for real data')
    return mockMatches
  }
}
